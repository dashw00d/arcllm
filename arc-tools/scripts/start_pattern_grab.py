...
Usage:
    python3 scripts/start_pattern_grab.py --entity-type 'wedding venues' --mission-id <uuid>
...
async def main():
    ...
    parser.add_argument("--mission-id", type=str, default=None,
                        help="Churner mission ID to bridge results into")
    ...
    if mission_id:
        try:
            ...
            log.error("--mission-id must be a valid UUID")
            return
    ...
    log.info("Starting PatternGrabWorkflow id=%s entity_type=%s mission_id=%s",
             wf_id, args.entity_type or "all", mission_id or "default")
